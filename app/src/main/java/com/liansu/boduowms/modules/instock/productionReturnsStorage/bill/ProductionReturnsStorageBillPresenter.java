 ...
 */
public class ProductionReturnsStorageBillPresenter extends BaseOrderBillChoicePresenter<IBaseOrderBillChoiceView, ProductionReturnsStorageBillModel> {

    public ProductionReturnsStorageBillPresenter(Context context, IBaseOrderBillChoiceView view, MyHandler<BaseActivity> handler, String businssType) {
        super(context, view, new ProductionReturnsStorageBillModel(context, handler));
    }


    @Override
    protected String getTitle() {
        return mContext.getResources().getString(R.string.appbar_title_production_returns_bill);
    }
...
//        mView.bindListView(list);
    ...
    public void getOrderHeaderList(OrderRequestInfo orderHeaderInfo) {
        mModel.requestOrderDetailList(orderHeaderInfo, new NetCallBackListener<String>() {
            @Override
