...
public class BaseOutStockBusinessModel extends BaseModel {
    ...
    OutStockOrderHeaderInfo   mOrderHeaderInfo;
...
//                    String batchNo = materialInfo.get() != null ? materialInfo.getBatchno() : "";
     ...
     * @time 2020/7/5 11:01
     */
    public BaseMultiResultInfo<Boolean, OutStockOrderDetailInfo> findMaterialInfo(OutBarcodeInfo scanFatherBarcode,OutBarcodeInfo scanSubBarcode,int scanType) {
        BaseMultiResultInfo<Boolean, OutStockOrderDetailInfo> resultInfo = new BaseMultiResultInfo<>();
        OutStockOrderDetailInfo sMaterialInfo = null;
        String barcodeMaterialNo = scanFatherBarcode.getMaterialno() != null ? scanFatherBarcode.getMaterialno() : "";
        String barcodeBatchNo = scanFatherBarcode.getBatchno() != null ? scanFatherBarcode.getBatchno() : "";
...
//                    String batchNo = materialInfo.getBatchno() != null ? materialInfo.getBatchno() : "";
     ...
     * @time 2020/7/5 13:40
     */
    public BaseMultiResultInfo<Boolean, Void> updateMaterialInfo(OutStockOrderDetailInfo detailInfo, OutBarcodeInfo scanBarcode, boolean isUpdate) {
                    ...
                    if (hasRemainQty > 0) {
                        if (ArithUtil.sub(hasRemainQty, barcodeScanQty) >= 0) {
                            if (isUpdate) {
                                ...
                                detailInfo.setOutstockqty(ArithUtil.sub(detailInfo.getVoucherqty(), detailInfo.getRemainqty()));
                            } else {
